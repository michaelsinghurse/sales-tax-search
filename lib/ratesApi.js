...
function findRateForType(object, type) {
  ...
  */
  ...
  if (type === "Combined") {
    ...
    let rateObjs = object.rates.filter(obj => obj.type === type) || [];
    if (rateObjs.length) {
      rateForType = rateObjs.reduce((total, obj) => {
        total += obj.rate;
        return total;
      }, 0);
    }
  } else {
    let rateObj = object.rates.find(obj => obj.type === type) || {};
    rateForType = rateObj.rate;
  }
